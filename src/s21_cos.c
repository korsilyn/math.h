...
long double s21_cos(double x) {
  ...
  if ((!S21_IS_INF(x)) && (!S21_IS_NAN(x))) {
    ...
    if ((x > -(S21_PI / 4)) && (x < (S21_PI / 4))) {
      calculate_cos_null(&res, minus, x, 8);
    }
    // cos(x) = - 1 * ((x - 90)^1 / 1!) + 1 * ((x - 90)^3 / 3!) - 1 * ((x -
    // 90)^5 / 5!) + 1 * ((x - 90)^7 / 7!)... Диапазон покрытия в градусах [45;
    // 90]
    if ((x >= (S21_PI / 4)) && (x <= (S21_PI / 2))) {
      calculate_cos_straight(&res, minus, x, 9, -1);
    }
...
void calculate_cos_null(long double* res, int minus, double x,
  ...
  for (int i = 0; i < num_of_cycles; i++) {
    ...
    // printf("%d_1) |%.15Lf|    |%.15f|\n", i, *res, tmp);
    if (((i % 2) == 0) && (i != 0)) *res = *res + tmp / factorial(2 * i);
    if ((i % 2) != 0) *res = *res - tmp / factorial(2 * i);
    // printf("%d_2) |%.15Lf|    |%.15f|\n", i, *res, tmp);
  }
  if (minus) *res = -(*res);
}
...
void calculate_cos_straight(long double* res, int minus, double x,
  ...
  for (int i = 1; i < num_of_cycles; i++) {
    double tmp = (x + sign * S21_PI / 2);
    for (int n = 1; n < i; n++)
      tmp = tmp * (x + sign * S21_PI / 2) * (x + sign * S21_PI / 2);
    // printf("%d_1) |%.15Lf|    |%.15f|\n", i, *res, tmp);
    if ((i % 2) == 0) *res = *res - sign * tmp / factorial(2 * i - 1);
    if (((i % 2) != 0) && (i != 1))
      *res = *res + sign * tmp / factorial(2 * i - 1);
    if (i == 1) *res = sign * tmp;
    // printf("%d_2) |%.15Lf|    |%.15f|\n", i, *res, tmp);
  }
  if (minus) *res = -(*res);
